...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    deconv_1 = tf.layers.conv2d_transpose(conv_1x1, num_classes, 4, strides = (2, 2), padding = 'SAME',
                                          kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    vgg_layer4_conv_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, strides = (1, 1), padding = 'SAME',
                                           kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    skip_1 = tf.add(deconv_1, vgg_layer4_conv_1x1)

    deconv_2 = tf.layers.conv2d_transpose(skip_1, num_classes, 4, strides = (2, 2), padding = 'SAME',
                                          kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    vgg_layer3_conv_1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, strides=(1, 1), padding='SAME',
                                           kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    skip_2 = tf.add(deconv_2, vgg_layer3_conv_1x1)

    output = tf.layers.conv2d_transpose(skip_2, num_classes, 16, strides = (8, 8), padding = 'SAME',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    return output
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        ...
        print('Epoch {}.....Cross_Entropy Loss = {}'.format(i, loss))
    pass
